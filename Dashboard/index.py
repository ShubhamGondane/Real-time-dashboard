...
client = bigquery.Client.from_service_account_json("scalable-ml-1fb18ed93052.json")


### BEGIN CREATE WORD CLOUDS ###
# Selects the corresponding words from the BigQUery table
query_words = ("SELECT Word FROM `scalable-ml.Dashboard_tables.Entity_raw` LIMIT 100")
...
query_authors = ("""SELECT Distinct(Author) FROM `scalable-ml.Dashboard_tables.Article_masterdata`
                    Order By Author""")
...
query_tags = ("""SELECT distinct(TS.Tag)
                    FROM `scalable-ml.Dashboard_tables.Article_masterdata` AS AM
                    inner join `scalable-ml.Dashboard_tables.Article_tags` AS TS on AM.ID = TS.ID
                    Order by TS.Tag""")
...
def update_output_top_article(value):
    if (value == "ALL"):
        ...
        query_authors = ("""SELECT Title, Author, Claps, No_Responses AS Responses, Reading_time AS ReadingTime, PublishingDate AS Date, ROW_NUMBER() OVER (ORDER BY Claps desc) As Id FROM `scalable-ml.Dashboard_tables.Article_masterdata`
